// --- THEME TOGGLE LOGIC ---
        ...
        function updateThemeBtn(isLight) {
            ...
            if (icon && text) {
                ...
                text.innerText = isLight ? 'DARK' : 'LIGHT';
            }
        }
        ...
        function renderLeaderboard() {
            ...
            currentProcessedUsers.forEach((u, i) => {
                ...
                const rankBadge = userRank ? `<span class="season-rank-badge" title="Season Rank">#${userRank}</span>` : '';
            ...
            globalSeasonStatsArray.forEach((p, i) => {
                ...
                const historyHtml = p.history.map(h => `
                    <div style="background: rgba(255,255,255,0.03); padding: 10px 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                        <span style="font-size: 0.8rem; font-weight: 700; color: var(--title-color);">${h.name}</span>
                        <div style="text-align: right;">
                            <span style="font-size: 0.75rem; color: var(--text-muted); margin-right: 12px; display: inline-block; width: 55px; text-align: left;">Pos: <strong style="color: var(--title-color);">${h.pos}</strong></span>
        ...
        function renderDraft(u, container) {
            ...
            container.innerHTML = dupWarning + u.lineup.map(g => `<div class="draft-item"><img src="${(g.avatar||g.player?.avatar) ? `https://media.realapp.link/assets/players/default/large/${g.avatar||g.player.avatar}.webp` : 'https://placehold.co/48x48/111/fff?text=?'}" class="draft-item-avatar" onerror="this.src='https://placehold.co/48x48/111/fff?text=?'"><span class="draft-item-name">${g.displayName||g.player?.displayName||'Unknown'}</span><span class="draft-item-score">${(parseFloat(g.score)||0).toFixed(2)}</span></div>`).join('');
        }
        ...
        function exportDay(dayName) {
            ...
            const d = new Date(activeTournamentStartDate);
            d.setDate(d.getDate() + targetDayIdx);
            const dateStr = (d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear();

            const tDates = [];
            for (let i = 0; i < 4; i++) tDates.push(getFormattedDate(activeTournamentStartDate, i));

            // 1. Determine prior day date string to subtract historical data
            let priorDayStr = null;
            if (targetDayIdx > 0) {
                const pd = new Date(activeTournamentStartDate);
                pd.setDate(pd.getDate() + targetDayIdx - 1);
                priorDayStr = (pd.getMonth()+1)+'/'+pd.getDate()+'/'+pd.getFullYear();
            }

            const tempPool = [];
            const tempUsers = users.map(u => {
                const normId = u.userId.toLowerCase().trim();
                const hist = historicalDailyDataByDate[normId] || {};
                
                // Cumulative raw points up to the PREVIOUS day
                const priorCumulative = (priorDayStr && hist[priorDayStr]) ? hist[priorDayStr].raw : 0;
                
                // The CURRENT live total points from the API. 
                // If a day was missed, this is still accurate for that missing day!
                const liveCumulative = u.hasData ? u.actualTotalPoints : 0; 
                
                // The points scored ONLY on the requested day
            ...
            tempUsers.forEach(x => {
                ...
                // Notice we completely ignore the current 'isWD' status!
                // We strictly build the median pool from anyone who actually scored points on the requested day.
            ...
            tempUsers.forEach(x => {
                if (x.u.hasData) {
                    ...
                    // We successfully export the live API total as "raw points" and the dynamically calculated day's Golf Score
                    rows.push([x.u.username, x.u.userId, x.liveCumulative.toFixed(2), golfScore, dateStr]);
                }
            });
            
            generateCSV(rows, `golf_plug_${dayName}_${dateStr.replace(/\//g, '-')}.csv`);
            document.getElementById('export-menu').style.display = 'none';
        }
        ...
        async function loadAllUserScores() {
                ...
                while (q.length > 0) {
                    ...
                    try {
                        ...
                        if (response.status === 429 || response.status === 502) {
                            ...
                            // 1. Recursively search entire JSON payload for the user's ID
                            ...
                            // 2. Fallback: If API omitted name from lineup, fetch profile directly!
                            if (!foundUsername) {
                                // Must seamlessly pace this request too so we don't cause a 429 spike
                                const now2 = Date.now();
                                let delay2 = 0;
                            ...
                            // 3. Mutate the Live users array directly!
                    ...
                    // Throttle DOM renders to prevent browser UI freezing
            ...
            // Launch the workers concurrently
            const workers = [];
            for (let i = 0; i < CONCURRENCY_LIMIT; i++) {
                workers.push(syncWorker());
            }
            
            // Wait until all workers finish processing the queue
            await Promise.all(workers);
            
            // Re-calculate rankings to instantly apply any dynamically fetched username changes
            calculateGlobalRanks();
            
            // Final render to guarantee all data is displayed once the queue finishes
            renderLeaderboard();
            renderProLeaderboard();
            ...
            // Fallback if no numeric game ID was ever extracted from the API responses
            if (!currentGameId) {
        ...
        async function fetchTournamentSchedule() {
            try {
                ...
                // Adjust "now" to Eastern Time and subtract 3 hours so days rollover at 3 AM ET
                const etString = new Date().toLocaleString("en-US", {timeZone: "America/New_York"});
                const now = new Date(etString);
                now.setHours(now.getHours() - 3);
                ...
                const etIdx = findIdx(h, ['event type', 'tier']); // Map the event type column
                ...
                const parseCurrency = (str) => {
                    ...
                    // Extract numbers, ignore words like "RAX"
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    if (etIdx !== -1 && r[etIdx] && r[etIdx].toString().toLowerCase().trim().includes('no cut')) {
                        isNoCut = true;
                    }
